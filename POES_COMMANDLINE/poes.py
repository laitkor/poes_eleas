...
def reporting(reportModNames, batch, wordlist, configInfo, runid):
    ...
    for mod in reportModNames:
        try:
            m = __import__(mod)
            f1 = (filename.split('/')[1]).split('.')[0]
            output_folder = ""
            print(f1)

            #print("REPORT MODULE",batch, wordlist, os.path.join(os.getcwd(),"output"), mod, configInfo, runid)
            #m.report(batch, wordlist, os.path.join(os.getcwd(),"output"), mod, configInfo, runid)
            #print("REPORT MODULE",batch, wordlist, os.path.join(os.getcwd(),"output/output-"+f1), mod, configInfo, runid)
            if mod == "score_data_csv":
                output_folder = "output/output-CSV"
            else:
                _folder = f1.split('_')[0]
                #output_folder = "output/output-"+f1
                output_folder = "output/"+_folder+"/"
            if not os.path.exists(os.path.join(os.getcwd(),output_folder)):
                os.makedirs(os.path.join(os.getcwd(),output_folder))                
            #print(output_folder)
            if mod == "score_data_csv":
                m.report(batch, wordlist, os.path.join(os.getcwd(),output_folder), mod, configInfo, runid,additionalInfo)
            else:
                m.report(batch, wordlist, os.path.join(os.getcwd(),output_folder), mod, configInfo, runid)
...
userInputs = [ # default value, description, command line flag, required
    ...
    ["wordlist", 'wordlist file location', '-w', False], 
    ...
    ["", 'additional info for CSV output files', '-p', False]
    ]
filename = ''
additionalInfo = ''
#print('SUBJECT', userInputs[0][0])
print("USER INPUTS \n",userInputs)
...
try:
    ...
    print("APPTXT")
    print(apptxt)
    print(inp[3].append(apptxt))
try:
    reqHR = sys.argv.index("-r")
    print('sys.argv.index("-r")',sys.argv.index("-r"))
except:
    pass
else:
    if inp[3]:
        ...
        print(inp[3].append(apptxt))
try:
    additionalInfo = sys.argv[sys.argv.index("-p")+1]
    print("sys.argv",sys.argv)
    print("additionalInfo",sys.argv[sys.argv.index("-p")+1])
except:
    pass
...
try:
    b = Batch()
    filename = userInputs[0][0]
    print('filename',filename)
    #print('SUBJECTS2',userInputs[0][0])
